...
private final class GiftViewSheetContent: CombinedComponent {
    ...
    final class State: ComponentState {
        ...
        private var overriddenSavedToProfile: Bool?
        ...
        func updateSavedToProfile(_ added: Bool) {
            ...
            self.overriddenSavedToProfile = added
        ...
        func openMore() {
            ...
            |> deliverOnMainQueue).start(next: { [weak self] peer in
                ...
                if reference == nil && !isDismantled {
                    items.append(.action(ContextMenuActionItem(text: savedToProfile ? strings.Gift_View_Hide : strings.Gift_View_Display, icon: { theme in
                        return generateTintedImage(image: UIImage(bundleImageName: savedToProfile ? "Chat/Context Menu/Hide" : "Chat/Context Menu/Show"), color: theme.contextMenu.primaryColor)
                    }, action: { [weak self] c, _ in
                        c?.dismiss(completion: nil)
                        self?.updateSavedToProfile(!savedToProfile)
                    })))
                }
    ...
    static var body: Body {
        ...
        return { context in
            ...
            if case let .soldOutGift(gift) = subject {
                ...
                var savedToProfile = arguments.savedToProfile
                if let overriddenSavedToProfile = state.overriddenSavedToProfile {
                    savedToProfile = overriddenSavedToProfile
                }
                ...
                } else if let title = genericGift?.title {
                    titleString = title
                } else if isSelfGift {
            ...
            } else if reference == nil && !isDismantled {
                let buttonTitle = savedToProfile ? strings.Gift_View_Hide : (isChannelGift ? strings.Gift_View_Display_Channel : strings.Gift_View_Display)
                buttonChild = button.update(
                    component: ButtonComponent(
                        background: buttonBackground,
                        content: AnyComponentWithIdentity(
    ...
    public enum Subject: Equatable {
        ...
        var arguments: (
            ...
            case let .starGift(gift):
                return (nil, nil, nil, nil, nil, nil, false, .generic(gift), 0, nil, nil, nil, false, false, nil, false, false, false, false, nil, nil, nil, nil, nil, nil, nil, nil, false, nil, nil, nil, nil)
            case let .profileGift(peerId, gift):
